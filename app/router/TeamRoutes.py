...
from app.utils.utils import display_members, getRandomCode
...
async def create_team(team: TeamCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db), background_tasks: BackgroundTasks = None):
    ...
    # Add the current users as an admin in the user_team_association table
    ...
    for email in team.member_emails:
        ...
        if user is not None:
            ...
            if existing_invitation is None:
                ...
                # generate a random code for each invitation
                code = secrets.token_urlsafe(6)
                invitation = Invitation(team_id=new_team.id, email=email, status='pending', code=code)
                db.add(invitation)
                db.commit()
                db.refresh(invitation)
                await send_team_invitation_email(email, background_tasks=background_tasks, team_name=new_team.name, code=invitation.code)
    ...
    return {"team": {
        ...
        "message": "un code d'invitation seront envoyés aux membres du groupe"}

@router.get("/invitations/accept")
def accept_invitation(code: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Check if the current users is the one who was invited
...
async def add_members(added_members: AddMembers, current_user: User = Depends(get_current_user),  db: Session = Depends(get_db), background_tasks: BackgroundTasks = None):
    ...
    for email in added_members.added_members_emails:
        ...
        if user is not None:
            ...
            if existing_invitation is None:
                code = secrets.token_urlsafe(6)
                invitation = Invitation(team_id=team_exists.id, email=email, status='pending', code=code)
                db.add(invitation)
                db.commit()
                db.refresh(invitation)
                await send_team_invitation_email(email, background_tasks=background_tasks, team_name=team_exists.name, code=invitation.code)
        else:
            not_registered_emails.append(email)
    ...
    return {
        ...
        "not_registered_emails": not_registered_emails,
        "message": "an invitation code will be sent to added members"
    }
...
def getTeamInfo(team: GetTeamInfo, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # verify that the users is member of the team
...
async def update_team(team_id: int, team_data: TeamUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # If the current users is not the admin of the team, return a 403 error
...
async def remove_members(team_id: int, remove_data: RemoveMembers, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for user_email in remove_data.user_emails:
        ...
        # If the users doesn't exist, return a 404 error
        if not user:
...
def get_team_info(team_name: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Verify that the users is a member of the team
    user_team = db.query(user_team_association).filter(user_team_association.c.user_id == current_user.id,
                                                      user_team_association.c.team_id == team_exists.id).first()
